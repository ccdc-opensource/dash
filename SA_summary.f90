...
! CSSR ...
 ...
 1010   FORMAT ('   ALPHA,BETA,GAMMA =',3F8.3,'    SPGR = ',A3)
        IF (PrefParExists) THEN
          WRITE (hFileCSSR,"(' ',I3,'   0 PO',3(1X,I3),X,F9.5)",ERR=999) TotNumOfAtoms,(PO_Direction(ii),ii=1,3),BestValuesDoF(iPrfPar,tRunNr)
        ELSE
...
! Add in a Header record
        ...
        IF (PrefParExists) THEN
          WRITE (hFilePDB,'(A,3(1X,I3),X,F9.5)',ERR=999) 'REMARK PO',(PO_Direction(ii),ii=1,3),BestValuesDoF(iPrfPar,tRunNr)
        ENDIF
...
! CCL ...
      IF (tSaveCCL) THEN
        ...
        IF (PrefParExists) THEN
          WRITE (hFileCCL,'(A,3(1X,I3),X,F9.5)',ERR=999) 'Z PO',(PO_Direction(ii),ii=1,3),BestValuesDoF(iPrfPar,tRunNr)
        ENDIF
...
! RES ...
      IF (tSaveRES) THEN
        ...
        IF (PrefParExists) THEN
          WRITE (hFileRES,'(A,3(1X,I3),X,F9.5)',ERR=999) 'REM  PO',(PO_Direction(ii),ii=1,3),BestValuesDoF(iPrfPar,tRunNr)
        ENDIF
