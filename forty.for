...
C>> JCC Was
C      SUBROUTINE FORTY(PNAME,ALSQ,MATSZ,PCXX,PFXX,MAGROU,CALROU,filnmr)
C>> Now
	INTEGER FUNCTION FORTY(PNAME,ALSQ,MATSZ,PCXX,PFXX,
     &                       MAGROU,CALROU,filnmr)
C>> The declaration here is changed to a function, to enable the return 
C
...
c
      ...
      USE druid_header
c
c.. Interaction here with Winteracter !!!!!!
c
...
C>> JCC add error handling declarations
	INTEGER IEOCC
	INTEGER PREFIN
C>> JCC This is for testing for mathematical errors in the CCSL that used to STOP the program
	INTEGER IBMBER
	COMMON / CCSLER / IBMBER 

	ICalled = 0

C>> JCC Initialise return value to successful (=1) any other value is failure
	FORTY = 1
      IPK = 0
      filnam_root=filnmr
...
C      write(76,*) ' About to enter PARSPR'
      CALL PARSPR(MAGROU)
C>> JCC Trap for any problems 
	IF ( IBMBER . GT. 0) GOTO 950
...
C FOR NOW, IMPOSE PHASE 1, SOURCE 1 WHICH WE HOPE STAY THERE:
      ...
      JSOURC=1
C
C.. CALCULATE PEAK & PEAK DERIVATIVE CONTRIBUTIONS
      IF (AKNOTS.LE.0.) THEN
        CALL PFXX(2) !PIKCAL
      ELSE
        CALL PFXX(3)
...
C..      CALL PFXX(3)
C
C...  2   READ (ISCR,END=3) ARGI,OBS,DOBS,WT,ICODE,KMIN,KMAX
C
...
C.. *** Winteracter call ***
      ...
      Npt30=Npts/30
C	write(76,*) ' About to cycle round ',npts,' obs'
      DO IPT=1,NPTS
...
C      COMMON /PRSAVF/PKLIST(512,9,200),ZXDEL(200),PKCONV(512,9),
...
C
 2    MN=512
      MN2=MN/2
C.. Peak positions may have changed - check and re-sort if necessary
      DO IR=1,MAXK
        KNOW=IR
...
C
C.. Set the number of peaks at each point to zero
      DO II=1,NPTS
        IOCCR(II)=0
      END DO
C
...
C
      GO TO 100
C
C PROFILE REFINEMENT STAGE with KNOTS:
C
 3    MN=512
      MN2=MN/2
C.. Peak positions may have changed - check and re-sort if necessary
...
C.. Now do the knots
      KOUNT=0
C      WRITE(76,*) ' There are ',AKNOTS,' knots'
      IF (AKNOTS.LE.1.) THEN
        KNOTS=AKNOTS*MAXK
      ELSE
        KNOTS=AKNOTS
...
C.. DESCRIBED IN FOURIER SPACE (GETS RID OF DISCONTINUITY PROBLEMS)
        CALL FCSUB3(MN)
        MN21=MN2-1
C	write(44,*) argk,know,mn
        DO II=1,MN
          PKKNOT(II,1,KNOW)=PKCONV(II,1)
          XDIFT=ZXDEL(1)*(II-MN21)
C	    write(44,*) XDIFT,PKCONV(II,1)
        END DO
C	WRITE(44,*) '*'
...
C
      ...
      DO KNOW=1,MAXK
	   ARGK=ZARGK(KNOW)
C.. Do interpolation ... and come out with PKCONV
C.. The first peak is at the first knot
C.. and the last peak at the last knot.
        JK=1 
        DO IK=JKNOT,KNOTS
 ...
 50     JKNOT=JK
     ...
     &    +C3FN0*PKKNOT(II,1,JK0)+C3FNP*PKKNOT(II,1,JK0+1)
        END DO
        DO JJ=1,NPKGEN(JPHASE,JSOURC)
          IF (PFNVAR(JJ,JPHASE,JSOURC)) THEN
            JJ1=JJ+1
     ...
     &    +   C3FN0*PKKNOT(II,JJ1,JK0)+C3FNP*PKKNOT(II,JJ1,JK0+1)
            END DO
          END IF
        END DO         
C.. FIND THE PEAK MAXIMUM VALUE AND THEN WORK OUT THE PEAK LIMITS
...
C
          DO I=1,3
            III=IARGI+I-2
C>> JCC Can get an array bound overflow error here so I've trapped for it temporarily. 
C>> May want to check out why ...
C>> Added a hack ...
...
C>> JCC Moved to an include file
	INCLUDE 'REFLNS.INC'
     ...
     1FR(512,8),FI(512,8),DR(512,8),DI(512,8),
     2FRT(512),FIT(512)
      LOGICAL PFNVAR
      COMMON /PFNINF/ NUMPFP(8,9,5),PFNVAR(8,9,5)
C
