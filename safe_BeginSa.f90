    ...
    SELECT CASE (ITYPE)
          CASE (PushButton)
           SELECT CASE (MESSAGE%VALUE1)
             CASE(IDF_SA_Play_Button)
!C>> JCC Add in check on files popup
			   IF (  CheckOverwriteSaOutput() .EQ. 1) THEN
!               Call WDialogFieldState(IDF_SA_Play_Button,Disabled)
				Call WDialogFieldState(IDF_SA_Play_Button,Enabled)
!
				Call WDialogFieldState(IDF_Pause_Annealing,Enabled)
				Call WDialogFieldState(IDF_SA_Stop_Button,Enabled)
!C>> JCC Added check on viewer
...
!>> Reselect!
				...
				Call WDialogHide()
			   END IF
             CASE(IDF_Pause_Annealing)
               CALL WMessageBox(OKOnly,ExclamationIcon,CommonOK, &
               'Press OK to continue','Pause')
             CASE(IDF_SA_Reanneal_Button)
!  Hide the current dialog
              CALL WDialogHide()
!  Return to the previous SA control dialog
              CALL WMessageEnable(FieldChanged,1)
              imyexit=3
             CASE(IDF_SA_NewAnneal_Button)
!  Hide the current dialog
              CALL WDialogHide()
!  Return to the previous SA control dialog
              CALL WMessageEnable(FieldChanged,1)
              imyexit=2 
             CASE(IDF_SA_Eject_Button)
!  Hide the current dialog
              CALL WDialogHide()
!  Return to the first SA control dialog
              CALL WMessageEnable(FieldChanged,1)
              imyexit=1                          
           END SELECT
   End Select
