...
! CSSR ...
 ...
 1010   FORMAT ('   ALPHA,BETA,GAMMA =',3F8.3,'    SPGR = ',A3)
        ...
        WRITE (hFileCSSR,1030,ERR=999) TemperatureStr, -SNGL(fopt), CHIPROBEST, ntotmov
...
! Add in a Header record
        ...
        WRITE (hFilePDB,1040,ERR=999) TemperatureStr, -SNGL(fopt), CHIPROBEST, ntotmov
...
! CCL ...
      IF (tSaveCCL) THEN
        ...
        WRITE (hFileCCL,1090,ERR=999) TemperatureStr, -SNGL(fopt), CHIPROBEST, ntotmov
...
! RES ...
      IF (tSaveRES) THEN
        ...
        WRITE (hFileRES,1031,ERR=999) TemperatureStr, -SNGL(fopt), CHIPROBEST, ntotmov
...
! NumOf_SA_Runs holds the number of completed multiruns
      ...
      INTEGER Limit1, Limit2, iAtom
      INTEGER tInteger, hFilePDB
      INTEGER, EXTERNAL :: WritePDBCommon
...
! In short: works only with Mercury.
      ...
      DO RunNr = 1, NumOf_SA_Runs
        IF (RangeOption .EQ. 1) THEN ! "Show Solutions p through q"
          UseThisSolution = ((RunNr .GE. Limit1) .AND. (RunNr .LE. Limit2))
        ELSE ! "Show Selected"
...
! Per Z-matrix, determine the connectivity. This has to be done only once.
      ...
      DO RunNr = 1, NumOfOverlaidStructures
        DO BondNr = 1, TotNumBonds
          WRITE(hFilePDB,'(A6,I5,I5)',ERR=999) 'CONECT', (pdbBond(BondNr,1)+ntatm*(RunNr-1)), (pdbBond(BondNr,2)+ntatm*(RunNr-1))
        ENDDO
      ENDDO ! loop over runs
      WRITE (hFilePDB,"('END')",ERR=999)
      CLOSE (hFilePDB)
