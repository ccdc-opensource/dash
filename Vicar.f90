...
!*****************************************************************************
!
! Same as above but for Rietveld refinement. Uses REAL rather than REAL*8
!
      SUBROUTINE RR_makexyz(n,blen,alph,bet,iz1,iz2,iz3,Cartesian)
...
! Loop over each atom in turn, finding its coordinates
      ...
      IF (k.LE.n) THEN
        DO i = k, n
          ...
          angle1 = alph(i)
          angle2 = bet(i)
          CALL RR_xyzatm(Cartesian,i,i1,bond,i2,angle1,i3,angle2)
        ENDDO
      ENDIF
  999 RETURN

      END SUBROUTINE RR_makexyz
!
!*****************************************************************************
!
      SUBROUTINE RR_xyzatm(Cartesian,i,i1,bond,i2,angle1,i3,angle2)
!
!     "xyzatm" computes the Cartesian coordinates of a single
!     atom from its defining internal coordinate values
...
! I have to brush up on my mathematics here: I would never have added this test.
      ...
      ELSE
        sinarg = MAX1(small,cosine**2-1.0)
        one_over_sine = 1.0/SQRT(sinarg)
      ENDIF
      u3 = u3 * one_over_sine
      u4(1) = u3(2)*u2(3) - u3(3)*u2(2)
