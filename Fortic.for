C
C
C LEVEL 14      SUBROUTINE FORTIC(PNAME,ALSQ,MATSZ,PCXX,PFXX,MAGROU,CALROU,FILNMR)
      SUBROUTINE FORTIC(PNAME,ALSQ,MATSZ,PCXX,PFXX,MAGROU,CALROU,FILNMR)
C
C *** FORTIC from FORTY - WIFD***
C
CX
CC 19B
...
C.. Check if we have too many reflections
	CALL CHKMAXREF(PCXX)
C
C
...
C
c           CALL ROTSYM(H,RH,I,-1)
c           F1=SCALPR(X(1,N),RH)+SCALPR(TRANS(1,I),H)
	do ir=1,maxk
	  do ii=1,3
	    h(ii)=irefh(ii,ir)
	  end do
